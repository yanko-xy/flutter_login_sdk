...
class LoginDao {
  ...
  static login({required String userName, required String password}) async {
    ...
    final response = await http.post(uri, headers: xyHeader());
  ...
  static void _saveUserInfo(
      {required boardingPass, required userName, required avatar, required imoocId}) {
    var map = {
      "boardingPadd": boardingPass,
      "avatar": avatar,
      "imoocId": imoocId,
      "userName": userName,
  ...
  static Map<String, dynamic>? getUserInfo() {
    var reslut = HiCache.getInstance().get(kUserInfo);
    if (reslut != null) {
      return jsonDecode(reslut);
    }
